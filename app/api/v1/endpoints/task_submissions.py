...
async def list_task_submissions(
    ...
    if not (is_super_or_admin(current) or is_teacher(current)):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Solo profesores y administradores pueden ver todas las entregas",
        )
    await _assert_enrolled_in_task_course(db, current, task_id)
...
async def get_my_submission(
    ...
    await _assert_enrolled_in_task_course(db, current, task_id)
    s = await task_submission_repository.get_by_task_and_user(
        db, task_id, current.id
    )
    if not s:
        return None
...
async def download_submission_file(
    ...
    s = await task_submission_repository.get_by_id(db, submission_id)
    if not s:
        raise HTTPException(
    ...
    if current.role == UserRole.student.value and s.user_id != current.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="No puedes descargar la entrega de otro estudiante",
        )

    settings = get_settings()
    try:
        client = get_minio_client(settings)
        data = await asyncio.to_thread(client.download_bytes, s.file_url)
    except S3Error as e:
